...
public class GraphBD<NodeType, EdgeType extends Number> implements GraphADT<NodeType, EdgeType> {
  ...
  protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
    PriorityQueue<Path> frontier = new PriorityQueue<Path>(); // contains Paths to be visited
    LinkedList<Vertex> visited = new LinkedList<Vertex>(); // a list of Paths that have been visited
    boolean found = false;
    ...
    // check for start or end not found
    if (startVertex == null || endVertex == null) {
      throw new NoSuchElementException("Either start or end could not be found");
    }
    ...
    while (!frontier.isEmpty()) {
      do {
        dequeued = frontier.peek();
        frontier.remove(); // the path is being visited
        newVertex = dequeued.end;
      } while (visited.contains(newVertex));

      // check if end is reached
      if (newVertex.equals(endVertex)) {
        return dequeued; // first path that reaches end
      }

      for (int i = 0; i < newVertex.edgesLeaving.size(); i++) {
        Path newPath = new Path(dequeued, newVertex.edgesLeaving.get(i)); // find all new paths
        frontier.add(newPath); // add each new path to the priority queue (frontier)
      }

      visited.add(newVertex); // track vertices that have already been visited
    }

    // no path exists
    if (!found) { // to avoid return below being unreachable
      throw new NoSuchElementException("No path from start to end could be found");
    }

    return null; // never reached -- exception is thrown every time while loop is exited
  }
  ...
  public List<String> getMST(String start) {
